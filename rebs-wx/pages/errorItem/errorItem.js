...
Page({
  ...
  //移除错题
  setRemoveList: function() {
    var that = this;
    //移除错题
    util.request(api.deleteErrorItem, {
      errorId: that.data.answers.allLists[that.data.answers.activeNum].errorId
      // employeeId: that.data.userInfo.employeeId,
      // type: that.data.type
    }, 'GET').then(function(res) {
      if (res.errno == 300) {
  ...
  insertErrorItem: function() {
    ...
    if(that.data.type == 2){
      errorData.examId = that.data.id;
    }
    util.request(api.insertErrorItem, errorData, 'POST').then(function(res) {
      if (res.errno == 300) {
        that.callBackError(res.errmsg);
      } else {
