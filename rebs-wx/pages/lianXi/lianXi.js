...
Page({
  ...
  getItemView: function () {
    ...
    util.request(api.getItemView, { chapterId: this.data.chapterId,bookId: this.data.bookId }).then(function (res) {
      // console.log(res);
      if (res.errno == 300) {
  ...
  setSwiperList() {
    ...
    util.request(api.isCollection, 
      { itemId: this.data.swiper.list[1].itemId, employeeId: that.data.userInfo.employeeId, type: 1 },'POST'
    ).then(function (res) {
      if (res.errno == 300) {
        that.callBackError(res.errmsg);
      }
  ...
  pageClick: function () {
    ...
    this.setData({ layerlayer: this.data.layerlayer});
  },
  //页码切换列表收缩
  ...
  setPricticeTime: function(){
    var that = this;
    //判断是否已经收藏
    util.request(api.setPricticeTime,
      {employeeId: that.data.userInfo.employeeId, beginTime: that.data.beginTime, endTime:that.data.endTime}, 'POST'
    ).then(function (res) {
      if (res.errno == 300) {
        that.callBackError(res.errmsg);
      }
      else {
        console.log('记录练习时长成功！');
        // wx.showModal({
        //   tip: '提示',
        //   content: '记录练习时长成功！',
        //   showCancel: false,
